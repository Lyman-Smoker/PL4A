...
def buid_models():
    patterns = build_pattern_miner(pattern_dim=1024, num_pattern=20)
    i3d = I3D_backbone(I3D_class=400, gap=False)
    cls_branch = Cls_Branch(out_dim=len(action_id_list), pattern_dim=1024, tau=1)
    rgs_branch = Rgs_Branch(dim=1024, topK=args.topk, dropout=args.drop_rate)
    return patterns, i3d, cls_branch, rgs_branch
...
def run_net():
    ...
    # models
    patterns, i3d, cls_branch, rgs_branch = buid_models()
    ...
    optimizer = optim.Adam([
        ...
        {'params': patterns},
        {'params': cls_branch.parameters()},
        {'params': rgs_branch.parameters()}
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step in steps:
            ...
            for data in tqdm(data_loader):
                ...
                # 2: pass cls branch
                a_hat, cls_result = cls_branch(feat=i3d_feats, patterns=patterns)
                # 3: pass rgs branch
                pred_score = rgs_branch(patterns=patterns, feature=i3d_feats, a_hat=a_hat)
